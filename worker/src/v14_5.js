import previousWorker from './v14_4.js';

const VERSION = '14.5.0';
const INJECT = '<script src="/emoji-picker-fix.js?v=20260917a"></script>';
...
async function injectEmojiFix(req, res) {
  if (!shouldInject(req, res)) return res;
  const html = await res.text();
  if (html.includes('/emoji-picker-fix.js')) return new Response(html, res);
...
export default {
  async fetch(req, env, ctx) {
    try {
      ...
      return await injectEmojiFix(req, res);
    } catch (error) {
      console.error('AINFO V14.5 wrapper error', error);
